...
if(!$selectAdmin || $selectAdmin == "all"){
	$result = $weeklyReportService->GetAllWeeklyReportsByPage(Config::$pageSize, $currentPage, 0, $currentAdmin->Id, 0, "2,3,4");
}else{
	$result = $weeklyReportService->GetAllWeeklyReportsByPage(Config::$pageSize, $currentPage, $selectAdmin, $currentAdmin->Id, 0, "2,3,4");
}

$recoudCount = $result[0];
$weeklyReportArray = $result[1];
$pager = new Pager(Config::$pageSize,$recoudCount,$currentPage,"approveWeekReportList.php?page=");
?>
...
<body>
	<div style="margin:3px;">
		<form id="form1" method="get" action="approveWeekReportList.php">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:200px; font-weight:bold">
					Approve week report list
				</td>
                <td>
					<b>Admin:</b>
					<select id="selectAdmin" name="selectAdmin" style="width:100px" onchange="weekReportListAdminChange()">
						<option value="all" <?php  if($selectAdmin == "all"){echo 'selected="selected"';}?>>--All--</option>
						<?php
						$allAdmins = $adminService->GetAllAdmins();
						for($i=0; $i < count($allAdmins); $i++)
						{
							if($allAdmins[$i]->Id == 8|| $allAdmins[$i]->Id == 26){
								continue;	
							}
							if($selectAdmin == $allAdmins[$i]->Id){
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; text-align:center; font-size:13px">
			...
			<tr style="background-color:#ffffff; color:#344b50;"
				...
				<td>
					<a href="weekReportApprove.php?page=<?php echo $currentPage ?>&selectAdmin=<?php echo $selectAdmin ?>&id=<?php echo $weeklyReportArray[$i]->Id ?>"><img alt="edit" src="../Images/b_edit.png" border="0"/></a>
