...
if($searchText){
	$_SESSION[sendBoxSearchType] = "searchText";
	$_SESSION[sendBoxSearchValue] = trim($searchText);
} else if($showAll){
	$_SESSION[sendBoxSearchType] = "showAll";
}

if($_SESSION[sendBoxSearchType] == "searchText"){
	$result = $replyMailInfoService->GetSendMailInfosByBusinessAdminIdAndPage($_SESSION[sendBoxSearchValue], $currentAdmin->Id, Config::$mailPageSize, $currentPage);
} else {
	$result = $replyMailInfoService->GetSendMailInfosByBusinessAdminIdAndPage("",$currentAdmin->Id, Config::$mailPageSize, $currentPage);
}
...
<body>
	<div style="margin:3px;">
		<form name="form1" method="post" action="sendBox.php">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:300px; font-weight:bold">
					...
					<?php 
					if($_SESSION[sendBoxSearchType] == "searchText"){
						echo "&nbsp;-Search: ".$_SESSION[sendBoxSearchValue];
					} else{
						echo "&nbsp;-Show all";
					}
					?>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; text-align:center; font-size:13px">
			<tr style="background-color:#d3eaef; font-weight:bold">
				...
				<td style="width:8%;">Sender</td>
				<td style="width:12%;">Reply time</td>
				<td style="width:12%;">Approve time</td>
				<td style="width:12%;">Send time</td>
			...
			<tr style="background-color:#ffffff; color:#344b50" 
				...
				<td><?php echo Common::ChangeDateFormat($replyMailInfoArray[$i]->ApprovalTime,"dateTime");?></td>
				<td><?php echo Common::ChangeDateFormat($replyMailInfoArray[$i]->SendTime,"dateTime");?></td>
				<td>
					<?php
