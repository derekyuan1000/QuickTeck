...
if($searchText){
	$_SESSION[mailApproveListSearchType] = "searchText";
	$_SESSION[mailApproveListSearchValue] = trim($searchText);
} else if($showAll){
	$_SESSION[mailApproveListSearchType] = "showAll";
}

if($_SESSION[mailApproveListSearchType] == "searchText"){
	$result = $replyMailInfoService->GetNeedApproveMailInfosByPage($_SESSION[mailApproveListSearchValue], Config::$mailPageSize, $currentPage);
} else {
	$result = $replyMailInfoService->GetNeedApproveMailInfosByPage("",Config::$mailPageSize, $currentPage);
}

$recoudCount = $result[0];
$replyMailInfoArray = $result[1];
$pager = new Pager(Config::$mailPageSize,$recoudCount,$currentPage,"mailApproveList.php?page=");
?>
...
<body>
	<div style="margin:3px;">
		<form name="form1" method="post" action="mailApproveList.php">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:300px; font-weight:bold">
					...
					<?php 
					if($_SESSION[mailApproveListSearchType] == "searchText"){
						echo "&nbsp;-Search: ".$_SESSION[mailApproveListSearchValue];
					} else{
						echo "&nbsp;-Show all";
					}
					?>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; text-align:center; font-size:13px">
			...
			<tr style="background-color:#ffffff; color:#344b50" 
				...
				<td>
					...
					if($replyMailInfoArray[$i]->MailStatus == 1){
					?>
					<a href="mailApprove.php?page=<?php echo $currentPage ?>&id=<?php echo $replyMailInfoArray[$i]->Id ?>"><img alt="edit" src="../Images/b_edit.png" border="0"/></a>
