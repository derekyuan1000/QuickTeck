		...
		<table width="1000px" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; font-size:12px; text-align:left">
			...
			<tr style="background-color:#ffffff; color:#344b50;">
				...
				<td style="height:500px">
					<?php  
						$filesize = 0;
						if (file_exists($_SERVER['DOCUMENT_ROOT']."/Internal/Mail/MailBody/".$mailInfo->Id."_Html.html")){
			...
			<tr style="background-color:#ffffff; color:#344b50;">
				...
				<td style="padding-left:6px;">
					<input id="isNeedApproval1" name="isNeedApproval" type="radio" value="1" <?php if($mailInfo->IsNeedApproval==1){echo 'checked="checked"';} ?>/>Yes&nbsp;&nbsp;
					<input id="isNeedApproval0" name="isNeedApproval" type="radio" value="0" <?php if($mailInfo->IsNeedApproval==0){echo 'checked="checked"';} ?>/>No
				</td>
			</tr>
			<tr style="background-color:#ffffff; color:#344b50;">
				<td style="height:25px;">
				...
				<td style="padding-left:6px;">
					<select id="ratedVitalityTime" name="ratedVitalityTime" style="width:120px">
						<option value="0" <?php if(!$mailInfo->RatedVitalityTime){echo 'selected="selected"';}?>>--Select--</option>
						<option value="30" <?php if($mailInfo->RatedVitalityTime == 30){echo 'selected="selected"';}?>>30 minutes</option>
						<option value="60" <?php if($mailInfo->RatedVitalityTime == 60){echo 'selected="selected"';}?>>1 hour</option>
						<option value="120" <?php if($mailInfo->RatedVitalityTime == 120){echo 'selected="selected"';}?>>2 hours</option>
						<option value="240" <?php if($mailInfo->RatedVitalityTime == 240){echo 'selected="selected"';}?>>4 hours</option>
						<option value="480" <?php if($mailInfo->RatedVitalityTime == 480){echo 'selected="selected"';}?>>8 hours</option>
					</select>
				</td>
			</tr>
			<tr style="background-color:#ffffff; color:#344b50;">
				...
				<td style="padding-left:6px;">
					<select id="businessAdminId" name="businessAdminId" style="width:120px">
					<option value="">--Select--</option>
					<?php
					$allBusinessAdmin1 = $adminService->GetOnDutyBusinessAdmin();
					//if(count($allBusinessAdmin) == 0){
					$allBusinessAdmin2 = $adminService->GetMailAdmin();	
					//}
					for($i=0; $i < count($allBusinessAdmin1); $i++)
					{
						if($mailInfo->BusinessAdminId == $allBusinessAdmin1[$i]->Id){
					?>
						<option value="<?php echo $allBusinessAdmin1[$i]->Id ?>" selected="selected"><?php echo $allBusinessAdmin1[$i]->Name ?></option>
					<?php
					}else{
					?>
						<option value="<?php echo $allBusinessAdmin1[$i]->Id ?>"><?php echo $allBusinessAdmin1[$i]->Name ?></option>
					<?php
						}
					}
					for($i=0; $i < count($allBusinessAdmin2); $i++)
					{
						if($mailInfo->BusinessAdminId == $allBusinessAdmin2[$i]->Id){
					?>
						<option value="<?php echo $allBusinessAdmin2[$i]->Id ?>" selected="selected"><?php echo $allBusinessAdmin2[$i]->Name ?></option>
					<?php
					}else{
					?>
						<option value="<?php echo $allBusinessAdmin2[$i]->Id ?>"><?php echo $allBusinessAdmin2[$i]->Name ?></option>
					<?php
						}
					}
					?>
				    ...
				    <table width="1000px" border="0" cellpadding="0" cellspacing="0">
				        <tr>
				            ...
				            <td style="width:20%; text-align:center">
				                <a href="../Do/doDeleteMailInfo.php?page=<?php echo $page ?>&id=<?php echo $id ?>" onclick="return confirm('Confirm to delete?')"><img alt="delete" src="../Images/b_delete.gif" border="0"/></a>
				            </td>
				            <td style="width:20%">
