...
if($searchText){
	$_SESSION[outBoxSearchType] = "searchText";
	$_SESSION[outBoxSearchValue] = trim($searchText);
} else if($showAll){
		$_SESSION[outBoxSearchType] = "showAll";
	}

if($_SESSION[outBoxSearchType] == "searchText"){
	$result = $replyMailInfoService->GetOutMailInfosByBusinessAdminIdAndPage($_SESSION[outBoxSearchValue], $currentAdmin->Id, Config::$mailPageSize, $currentPage);
} else {
	$result = $replyMailInfoService->GetOutMailInfosByBusinessAdminIdAndPage("", $currentAdmin->Id, Config::$mailPageSize, $currentPage);
}

$recoudCount = $result[0];
$replyMailInfoArray = $result[1];
$pager = new Pager(Config::$mailPageSize,$recoudCount,$currentPage,"outBox.php?page=");
?>
...
<body>
	<div style="margin:3px;">
		<form name="form1" method="post" action="outBox.php">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:300px; font-weight:bold">
					...
					<?php 
					if($_SESSION[outBoxSearchType] == "searchText"){
						echo "&nbsp;-Search: ".$_SESSION[outBoxSearchValue];
					} else{
						echo "&nbsp;-Show all";
					}
					?>
