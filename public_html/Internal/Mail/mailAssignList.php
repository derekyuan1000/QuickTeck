...
$searchNeedAssign = $_POST[searchNeedAssign];
$searchMailFlag = $_POST[searchMailFlag];
$showAll = $_POST[showAll];

if($searchNeedAssign){
	$_SESSION[mailAssignListSearchType] = "searchNeedAssign";
	$_SESSION[mailAssignListSearchValue] = $searchNeedAssign;
}else if($searchMailFlag){
	$_SESSION[mailAssignListSearchType] = "searchMailFlag";
	$_SESSION[mailAssignListSearchValue] = $searchMailFlag;
}else if($searchText){
	$_SESSION[mailAssignListSearchType] = "searchText";
	$_SESSION[mailAssignListSearchValue] = trim($searchText);
}else if($showAll){
	$_SESSION[mailAssignListSearchType] = "showAll";
}

if($_SESSION[mailAssignListSearchType] == "searchNeedAssign"){
	$result = $mailInfoService->GetAllMailInfosByPage('', Config::$mailPageSize, $currentPage, 1, 0);
}else if($_SESSION[mailAssignListSearchType] == "searchMailFlag"){
	$result = $mailInfoService->GetAllMailInfosByPage('', Config::$mailPageSize, $currentPage, 0, $searchMailFlag);
}else if($_SESSION[mailAssignListSearchType] == "searchText"){
	$result = $mailInfoService->GetAllMailInfosByPage($_SESSION[mailAssignListSearchValue], Config::$mailPageSize, $currentPage, 0, 0);
}else{
	$result = $mailInfoService->GetAllMailInfosByPage("",Config::$mailPageSize, $currentPage, 0, 0);
}

$recoudCount = $result[0];
$mailInfoArray = $result[1];
$pager = new Pager(Config::$mailPageSize,$recoudCount,$currentPage,"mailAssignList.php?page=");
?>
...
<body>
	<div style="margin:3px;">
		<form name="form1" method="post" action="mailAssignList.php">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:300px; font-weight:bold">
					Mail assign
					<?php 
					if($_SESSION[mailAssignListSearchType] == "searchText"){
						echo "&nbsp;(Search: ".$_SESSION[mailAssignListSearchValue].")";
					} else if($_SESSION[mailAssignListSearchType] == "searchNeedAssign"){
						echo "&nbsp;-Need assign";
					} else if($_SESSION[mailAssignListSearchType] == "searchMailFlag"){
						echo "&nbsp;-Mail flag: "; if($_SESSION[mailAssignListSearchValue]==1){echo "White";}else if($_SESSION[mailAssignListSearchValue]==2){echo "Yellow";}else{echo "Red";}
					} else{
						echo "&nbsp;(Show all)";
					}
					?>
				...
				<td style="width:750px;">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<b>Need assign:&nbsp;</b>
					<input id="searchNeedAssign" name="searchNeedAssign" type="checkbox"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<b>Mail flag:&nbsp;</b>
					<select id="searchMailFlag" name="searchMailFlag">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; text-align:center; font-size:13px">
			...
			<tr style="background-color:#ffffff; color:#344b50" 
				...
				<td>
					<a href="mailAssign.php?page=<?php echo $currentPage ?>&id=<?php echo $mailInfoArray[$i]->Id ?>"><img alt="edit" src="../Images/b_edit.png" border="0"/></a>
					&nbsp;&nbsp;|&nbsp;&nbsp;<a href="../Do/doDeleteMailInfo.php?page=<?php echo $currentPage ?>&id=<?php echo $mailInfoArray[$i]->Id ?>" onclick="return confirm('Confirm to delete?')"><img alt="delete" src="../Images/b_drop.png" border="0"/></a>
				</td>
			</tr>
			<?php
			}
