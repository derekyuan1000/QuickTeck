...
if($searchMailFlag){
	$_SESSION[inBoxSearchType] = "searchMailFlag";
	$_SESSION[inBoxSearchValue] = $searchMailFlag;
}else if($searchText){
	$_SESSION[inBoxSearchType] = "searchText";
	$_SESSION[inBoxSearchValue] = trim($searchText);
}else if($showAll){
	$_SESSION[inBoxSearchType] = "showAll";
}

if($_SESSION[inBoxSearchType] == "searchMailFlag"){
	$result = $mailInfoService->GetMailInfosByBusinessAdminIdAndPage('', $currentAdmin->Id, Config::$mailPageSize, $currentPage, $searchMailFlag);
}else if($_SESSION[inBoxSearchType] == "searchText"){
	$result = $mailInfoService->GetMailInfosByBusinessAdminIdAndPage($_SESSION[inBoxSearchValue], $currentAdmin->Id, Config::$mailPageSize, $currentPage, 0);
}else {
	$result = $mailInfoService->GetMailInfosByBusinessAdminIdAndPage("", $currentAdmin->Id, Config::$mailPageSize, $currentPage, 0);
}
	...
	<div style="margin:3px;">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#353c44; color:#e1e2e3;">
            <tr>
                ...
                <td style="width:300px; font-weight:bold">
					Inbox
					<?php 
					if($_SESSION[inBoxSearchType] == "searchText"){
						echo "&nbsp;(Search: ".$_SESSION[inBoxSearchValue].")";
					} else if($_SESSION[inBoxSearchType] == "searchMailFlag"){
						echo "&nbsp;-Mail flag: "; if($_SESSION[inBoxSearchValue]==1){echo "White";}else if($_SESSION[inBoxSearchValue]==2){echo "Yellow";}else{echo "Red";}
					} else{
						echo "&nbsp;(Show all)";
					}
					?>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" style="background-color:#a8c7ce; text-align:center; font-size:13px">
			<tr style="background-color:#d3eaef; font-weight:bold">
				...
				<td style="width:12%;">Assign time</td>
				<td style="width:8%;">Vitality(min)</td>
			...
			<tr style="background-color:#ffffff; color:#344b50" 
				...
				<td style="text-align:left;">
					...
					&nbsp;&nbsp;&nbsp;
					<?php } ?>
					<?php echo Common::CutString($mailInfoArray[$i]->Subject, 75)?>
				</td>
				<td><?php echo Common::ChangeDateFormat($mailInfoArray[$i]->MailTime,"dateTime"); ?></td>
				<td><?php echo Common::ChangeDateFormat($mailInfoArray[$i]->AssignTime,"dateTime");?>
				<td>
				<?php 
