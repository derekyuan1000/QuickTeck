...
if (isset($_POST['submit']) && !empty($_POST['payment_value']) && is_numeric($_POST['payment_value'])) {
	...
	if ($payment_value < $minimum_value || $payment_value > $maximum_value) {
		...
		$sagepay_server->setReference($_POST['ordernumber']."-".$payment_value*100);
...
<head>
<title>Printed circuit board manufacturer low cost,UK PCB circuit board manufacturer,PCB prototype manufacturing,PCB board manufacturer,make PCB</title>
<meta name="description" content="Order PCB from UK printed circuit board manufacturer,offering prototype PCB circuit board to multilayer volume production PCBs at low price,online quote for make circuit board." />
<meta name="keywords" content="Order PCB from UK printed circuit board manufacturer,prototype PCB circuit board to multilayer volume production circuit board at low price,online quote for make circuit board." />
...
<a href="http://www.facebook.com/quickteck"  target="_blank"><img src="/rescources/images/img2.gif" width="30" height="30" alt="" /></a></p>
		...
		<div class="left_box_b"></div>
				...
				<tr>
					<td class="style3" style="width: 170px"><span class="style13">*</span><?php Lang::EchoString2($langdate['manualpayment'][4]);?>(&pound):</td><td class="style3"><input type="number" name="payment_value" step="0.01" value="<?php echo $payment_value ?>" style="width: 100px"/></td>
				</tr>
				<tr>
