...
if($orderId != "" && $taskId != ""){
	...
	if($order && $_GET[key]==$order->Remark){
		...
		echo "<script type='text/javascript'>location.href='../clients/errorResetPassword_client.php';</script>";
		return;
	}
}else{
	$_SESSION[message] = "<span style='color:red; font-size:16px'>Invalid link!</span>";
        ...
        <tr> 
          <td class="style31" style="padding-left:20px">
			<div id="divAdminLogin">
				<form name="form1" method="post" action="doPostcodeLogin.php">
					<input id="taskId" name="taskId" type="hidden" value="<?php echo $taskId; ?>"/>
					<input id="orderId" name="orderId" type="hidden" value="<?php echo $orderId; ?>"/>
					<div style="font-size: medium; color: #FF6600; font-weight:bold; text-align:center; height:25px">Order confirm</div>
					<table width="450px" border="0" cellpadding="7" cellspacing="0">
						<tr style="background-color:#D7D59D">
							<td style="text-align:right; font-weight:bold; color:#0000FF; font-size:14px;">
								<b>&nbsp;Order number:</b>
							</td>
						...
						<tr style="background-color:#D7D59D">
                            ...
                            <td style="text-align:left">
                                <input id="postcode" name="postcode" type="text" class="myInput"/>
                            </td>
						</tr>
						<tr style="background-color:#D7D59D">
							<td colspan="2" align="center">
								<input type="submit" value="Login" class="button" onclick="return checkPostcodeLogin()"/>
							</td>
						</tr>
						<tr>
